...
firstLevelMenuItems.forEach(key => {
    ...
    if (key === 'dataperceive') meunItems.push(getMenuItem(label, key, getSecondaryMenuItem(key)))
    else meunItems.push(getMenuItem(
...
firstLevelMenuItems.forEach(firstLevelMeniItem => {
    let routeConfig
    if (firstLevelMeniItem === 'dataperceive') {
        routeConfig = secondaryMenuItems.map(secondaryMenuItem => {
            return {
            pathname: `${firstLevelMeniItem}/${secondaryMenuItem}`,
            componentPath: getComponentPath(firstLevelMeniItem),
            }
        });
    } else {
        routeConfig =[{
        pathname: `${firstLevelMeniItem}`,
        componentPath: getComponentPath(firstLevelMeniItem),
        }]
    }
